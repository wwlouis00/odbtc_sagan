...
def conv2d(input_, output_dim, 
           k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02,     # before k_h = 5, k_w = 5
           name="conv2d"):
    with tf.variable_scope(name):
...
def deconv2d(input_, output_shape,
    ...
    with tf.variable_scope(name):
        ...
        #     # deconv = tf.nn.conv2d_transpose(input_, w, output_shape=output_shape,
        #     #                     strides=[1, d_h, d_w, 1])
        #     deconv = tf.nn.conv2d_transpose(input_, w, output_shape=input_.get_shape()[-1],
        #                         strides=[1, d_h, d_w, 1])
        #     # print(deconv.shape)
        ...
        # biases = tf.get_variable('biases', [output_shape[-1]], initializer=tf.constant_initializer(0.0))
        biases = tf.get_variable('biases', dd, initializer=tf.constant_initializer(0.0))
        # print(deconv.shape)
        # print(biases.shape)
